...
// PublisherDetailPage: fetches and shows details for a specific publisher from the RAWG API
function PublisherDetailPage() {
  // Get the id from the URL params (e.g., /publisher/123)
  const { id } = useParams<{ id: string }>();
  // useNavigate hook for navigation
  const navigate = useNavigate();

  // State variables for loading, error, publisher data, games, pagination, and next page availability
  const [loading, setLoading] = useState(true); // Loading indicator
  const [error, setError] = useState<string | null>(null); // Error message
  const [publisher, setPublisher] = useState<any>(null); // Publisher details
  const [games, setGames] = useState<any[]>([]); // List of games by this publisher
  const [page, setPage] = useState(1); // Current page number
  const [hasNextPage, setHasNextPage] = useState(false); // If there is a next page

  // Fetch publisher details and their games from the API when id or page changes
  useEffect(() => {
    setLoading(true); // Start loading
    setError(null); // Reset error
    setPublisher(null); // Reset publisher
    setGames([]); // Reset games
    // Fetch publisher details
    fetch(`${API_BASE_URL}/publishers/${id}?key=${API_KEY}`)
      .then(res => {
        if (!res.ok) throw new Error('Failed to fetch publisher details'); // Handle HTTP errors
        return res.json(); // Parse JSON response
      })
      .then(data => {
        setPublisher(data); // Set publisher data
    ...
    // Fetch games for this publisher
    fetch(`${API_BASE_URL}/games?key=${API_KEY}&publishers=${id}&page_size=${PAGE_SIZE}&page=${page}`)
      .then(res => res.json())
      .then(data => {
  ...
  // Render the publisher detail page
  return (
    <div className="page-container">
      ...
      <header className="page-header">
        <div className="page-header-content">
          ...
          <div className="page-subtitle">Publisher Details</div>
        </div>
      </header>
      {/* Navigation bar with back button and publisher name */}
      <nav className="page-nav">
        <div className="page-nav-content">
          {/* Back to publishers button */}
          <button onClick={() => navigate('/publishers')} className="btn-consistent btn-consistent-secondary">
            ← Back to Publishers
          </button>
          {/* Publisher name centered */}
          <h2 style={{ margin: 0, color: 'var(--text-primary)', fontSize: '1.5rem', textAlign: 'center', width: '100%' }}>{publisher?.name}</h2>
        </div>
      </nav>
      {/* Main content area */}
      <main className="page-main">
        <div className="page-main-content">
          ...
          <div className="consistent-card" style={{ marginBottom: '2rem' }}>
            ...
            <div className="consistent-card-content">
              <h2 className="consistent-card-title" style={{ fontSize: '1.5rem' }}>{publisher?.name}</h2>
              <div className="consistent-card-meta">
                <p><strong>Games Count:</strong> {publisher?.games_count}</p>
              </div>
          ...
          <h3 style={{ marginBottom: '1rem' }}>Games by {publisher?.name}</h3>
