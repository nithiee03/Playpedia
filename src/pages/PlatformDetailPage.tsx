...
function PlatformDetailPage() {
  // Get the id from the URL params (e.g., /platform/123)
  const { id } = useParams<{ id: string }>();
  // useNavigate hook for navigation
  const navigate = useNavigate();

  // State variables for platform details, games, loading, error, search, pagination, and next page availability
  const [platform, setPlatform] = useState<any>(null); // Platform details
  const [games, setGames] = useState<any[]>([]); // List of games on this platform
  ...
  // Fetch platform details and games from RAWG API when id, search, or page changes
  useEffect(() => {
    setLoading(true); // Start loading
    setError(null); // Reset error
    setPlatform(null); // Reset platform
    setGames([]); // Reset games
    // Fetch platform details
    fetch(`${API_BASE_URL}/platforms/${id}?key=${API_KEY}`)
      .then(res => {
        if (!res.ok) throw new Error('Failed to fetch platform details'); // Handle HTTP errors
    ...
    <div className="page-container">
      ...
      <nav className="page-nav">
        <div className="page-nav-content">
          ...
          <button onClick={() => navigate('/platforms')} className="btn-consistent btn-consistent-secondary">
            ← Back to Platforms
          </button>
          {/* Section title */}
          <h2 style={{ margin: 0, color: 'var(--text-primary)', fontSize: '1.5rem' }}>{platform ? `${platform.name} Games` : 'Platform Games'}</h2>
